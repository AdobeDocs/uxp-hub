...
Indicates the version of the manifest schema. Current UXP hosts use manifest version `5`.
...
**Only during development**, for convenience, you can assign to the `host` property an array of `HostDefinition` objects, allowing the plugin to be loaded in multiple applications simultaneously. See the [Package with the UXP Developer Tool](../../../how-to/distribution/package/index.md#host-applications) section for more details.
...
Addon definitions for [Hybrid Plugins](../../../how-to/hybrid-plugins/index.md). A UXP Hybrid plugin is a UXP plugin that can load dynamically-linked C++ native libraries (`.uxpaddon` files) at runtime. See the [Hybrid Plugins guide](../../../how-to/hybrid-plugins/index.md) for details on building and configuring addons.
...
A description of the entrypoint. This description is used in tooltips and other places where a longer description is appropriate, depending on the host application. Default value is the plugin's name.
...
Keyboard shortcuts are not available in every UXP host yet. Premiere, for example, doesn't support them.
...
If your shortcut collides with a built-in command in the host application, or another plugin's shortcut, your shortcut will be ignored, and you'll see a warning printed to the developer console.
...
| Name  | Type                                          | Required |
| :---- | :---------------------------------------------- | :------- |
| `app` | `"PS"` or `"ID"` or `"premierepro"` or `"ame"` | required |

The host application that the plugin supports. Possible values are:
...
Code samples across these guides show `app: premierepro` as an example. Substitute the `app` value for your target host (`PS`, `ID`, `premierepro`, or `ame`) — the rest of the manifest structure stays the same.
...
The minimum version of the host application that the plugin supports in the `x.y.z` format.
...
The maximum version of the host application that the plugin supports in the `x.y.z` format. Default value is the latest version of the host application.
...
Enables the plugin to read and write to the clipboard. The [clipboard recipe](../../../how-to/recipes/clipboard/index.md) has more examples.
...
Enables the plugin to access the filesystem. The [Filesystem Operations recipe](../../../how-to/recipes/filesystem-operations/index.md) has detailed examples.
...
Default value is `undefined` (no network access). See the [`NetworkPermission`](#networkpermission) section for more details, or the [network recipe](../../../how-to/recipes/network/index.md) for examples.
...
Default value is `undefined` (no process launching). See the [`LaunchProcessPermission`](#launchprocesspermission) section for more details and the [launch process recipe](../../../how-to/recipes/external-process/index.md) for examples.
...
Enables the plugin to load native C++ addons (`.uxpaddon` files). This permission is required for [Hybrid Plugins](../../../how-to/hybrid-plugins/index.md). Default value is `false`.
...
The [network recipe](../../../how-to/recipes/network/index.md) has more details.
...
The [launch process recipe](../../../how-to/recipes/external-process/index.md) has more details.
...
Enables the plugin to communicate with other plugins. The [inter plugin communication guide](../../../how-to/inter-plugin-comm/index.md) has more details.

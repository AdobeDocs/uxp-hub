...
Learn how to declare and implement plugin entrypoints for commands and panels.
...
The `manifest.json` file is located at the root of the plugin bundle that contains all sorts of metadata about the plugin. Among all its properties, documented in [this reference guide](../manifest/index.md), Entrypoints are declared in the required `entrypoints` array.
...
The `hide()` and `destroy()` hooks don't work as expected yet in every host. Check your target host's release notes if a panel isn't tearing down the way you'd expect.
...
Command handlers are functions that are invoked when a command entrypoint is clicked in the host's plugins menu (in Premiere, **Window** > **UXP Plugins** > **`<your plugin name>`** > **`<your command name>`**).
